/**
 * Default settings for the Telegram Mini App
 * These values will be used for initialization and reset functionality
 */
...
/**
 * Get default settings as a map for quick lookup
 */
export function getDefaultSettingsMap(): Map<string, DefaultSetting> {
  const map = new Map<string, DefaultSetting>();
...
/**
 * Get default settings filtered by category
 */
export function getDefaultSettingsByCategory(category: string): DefaultSetting[] {
  return DEFAULT_SETTINGS.filter((s) => s.category === category);
}
